            ...
            {["all", ...categorys].map((c, i) => (
              <li key={i}>
                <button
                  ...
                  style={{
                    borderColor: c === category ? "var(--clr-orange)" : "",
                  }}
                >
                  {c}
                </button>
      ...
      <div className="side-right w-full">
            ...
            {colors.map((clr, i) => (
              <li key={i} className="flex">
                <button
                  style={{
                    backgroundColor: clr,
                    boxShadow: color === clr ? `0 0 10px 1px ${clr}` : "",
                  }}
        ...
        <label className="flex items-center gap-1 hover:opacity-80 cursor-pointer">
          ...
          <input
            ...
            onChange={freeShippingHandler}
          />
        </label>
        <button className="btn-fire w-full" onClick={clearHandler}>
          clear fillter
        </button>
      </div>
    </section>
