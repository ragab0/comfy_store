    ...
    <div>
          ...
          <figcaption className="flex-1 grid grid-cols-[1fr_auto] gap-8">
            <div>
              ...
              <button
                ...
                className="hover:opacity-100 transition-none opacity-80"
              >
                remove
              </button>
            </div>
      ...
      <div className="border-t-2">
        <h4 className="mt-4 mb-8 max-w-full flex items-center justify-between">
          total{" "}
          <span>${cartList.reduce((p, c) => p + c.price * c.count, 0)}</span>
        </h4>
        {cartList.length ? (
          <button onClick={clearHandler} className="btn-fire mx-auto !block">
            clear cart
          </button>
        ) : (
