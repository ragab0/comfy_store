  ...
  return (
    <main className="container grid md:grid-cols-2 justify-between gap-[5%] md:justify-center mb-20">
        ...
        >
          {[image, ...images].map((r, i) => (
            <Image
              ...
              width={75}
              height={75}
              className="w-full h-full rounded-md cursor-pointer hover:scale-90"
              onClick={(e) => currentImgHandler(e, i)}
            />
          ))}
        </div>
      </div>
      <div>
        ...
        <span className="price text-clrOrangeDark font-bold text-2xl spacing block my-2">
          ${price}
        </span>
        ...
        <div className="border-b-0 my-4">
          {[
            ...
            <p key={i} className="mb-2 text-base">
              <span className="w-[100px] inline-block font-bold text-clrHead2">
                {n}:
              </span>{" "}
              {`${v}`}
            </p>
            ...
            {colors.map((clr, i) => (
              <li key={i} className="flex">
                <button
                  style={{
                    backgroundColor: clr,
                    boxShadow: currentColor === clr ? `0 0 10px ${clr}` : "",
                  }}
                  onClick={() => currentColorHandler(clr)}
        ...
        <section>
          <div className="flex items-center text-2xl text-clrHead2 font-bold">
            <button
              className="p-1 hover:scale-150"
              onClick={increNumberHandler}
            >
              +
            </button>
          ...
          <button
            onClick={addHandler}
            className="btn-second mt-6 !rounded-none"
          >
            add to cart
          </button>
        </section>
