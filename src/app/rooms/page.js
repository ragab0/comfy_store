...
function RoomsBody() {
  ...
  const {
    rooms: { list, currentList, isLoading, isInitialized, error },
  } = useSelector((state) => state.roomsReducer);
  const { search, category, company, color, price, isFreeShipping } =
  ...
  useEffect(
    function () {
      ...
      if (sort === "az") {
        newCurrentList.sort((second, first) =>
          second.name.localeCompare(first.name)
        );
      } else if (sort === "za") {
        newCurrentList.sort((first, second) =>
          second.name.localeCompare(first.name)
        );
      } else if (sort === "p1") {
    ...
    <div className="body container mb-20 grid md:grid-cols-[auto_1fr] gap-8">
      ...
      <div>
        ...
        {!isInitialized || isLoading ? (
          <Loading />
        ) : (isInitialized && error) || !currentList?.length ? (
          <h1 className=" text-center">{"There is no Rooms to show !!"}</h1>
        ) : (
